...
def prev_day_link(year: int, month: int, day: int) -> str:
    ...
    return (
        f"/{previous_day_link.year}/{previous_day_link.month}/{previous_day_link.day}"
    )
...
def next_day(year, month, day):
    next_day = datetime.date(int(year), int(month), int(day)) + datetime.timedelta(
        days=1
    )
...
def next_week(year, month, day):
    next_week = datetime.date(int(year), int(month), int(day)) + datetime.timedelta(
        days=7
    )
