...
from func.calendar_func import (
    prev_month_link,
    next_month_link,
    previous_week_link,
    next_week_link,
...
def calendar_week_page(year=None, month=None, week=None) -> make_response:
    ...
    current_day, current_month, current_year = GregorianCalendar.current_date()
    current_week = GregorianCalendar.num_week_in_month_by_date(
    ...
    week_days_num = [
        int(week_days[x].strftime("%Y-%m-%d").split("-")[-1]) for x in range(7)
    ]
    ...
    week_events = [
        [
            dt.date(events[x][0], events[x][1], events[x][2]),
            events[x][3],
            events[x][4],
            events[x][0],
            events[x][1],
...
def calendar_day_page(year=None, month=None, day=None) -> make_response:
    ...
    weekday_name = current_app.config["WEEK_DAYS"][
        GregorianCalendar.num_weekdays(year, month, day)
    ]
    ...
    events = [
        [events[x][0], events[x][1], int(events[x][2]), events[x][3]]
        for x in range(len(events))
    ]
    return make_response(
        render_template(
...
def get_Info(day=None, month=None, year=None, num_hour=None):
    ...
    if request.method == "GET":
        return render_template(
            "addEvent.html", day=day, month=month, year=year, num_hour=num_hour
        )
        ...
        if request.form["submit"] == "AI":
            ...
            if freequency != "":
                ...
                if freequency == "everyday":
                    for x in range(d_num):
                        n_dates.append(
                            cf.next_day(n_dates[-1][0], n_dates[-1][1], n_dates[-1][2])
                            + [num_hour]
                        )
                elif freequency == "everyweek":
                    for x in range(d_num):
                        n_dates.append(
                            cf.next_week(n_dates[-1][0], n_dates[-1][1], n_dates[-1][2])
                            + [num_hour]
                        )
                elif freequency == "everyyear":
                    for x in range(d_num):
                        n_dates.append(
                            cf.next_year(n_dates[-1][0], n_dates[-1][1], n_dates[-1][2])
                            + [num_hour]
                        )
            ...
            return redirect(
                f"/calendar/event/{year}/{month}/{day}/{num_hour}/{events[-1][3]}"
            )
...
def viewNote(year=None, month=None, day=None, time=None, id=None):
    ...
    if request.method == "GET":
        ...
        return make_response(
            render_template(
                "event.html", event=res, year=year, month=month, day=day, time=time
            )
        )
    elif request.method == "POST":
        if request.form["submit"] == "save":
            ...
            return make_response(
                redirect(f"/calendar/event/{year}/{month}/{day}/{time}/{id}")
            )
