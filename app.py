...
def create_app(config_overrides: Optional[Dict] = None) -> Flask:
    ...
    app.add_url_rule(
        "/calendar/month/<year>/<month>/", "calendar_month_page", calendar_month_page
    )
    app.add_url_rule("/calendar/week/", "calendar_week_page", calendar_week_page)
    app.add_url_rule(
        "/calendar/week/<year>/<month>/<week>/",
        "calendar_week_page",
        calendar_week_page,
    )
    app.add_url_rule(
    ...
    app.add_url_rule(
        "/checkForLogin", "is_Logined", is_Logined, methods=["GET", "POST"]
    )
    ...
    app.add_url_rule(
        "/calendar/day/<day>/<month>/<year>/<num_hour>/add",
        "add",
        get_Info,
        methods=["GET", "POST"],
    )
    app.add_url_rule(
        "/change_password", "ch_p", change_password, methods=["GET", "POST"]
    )
    app.add_url_rule(
        "/check_for_change", "ch_check", check_for_change, methods=["GET", "POST"]
    )
    app.add_url_rule("/change", "change", change, methods=["GET", "POST"])
    app.add_url_rule(
        "/calendar/event/<year>/<month>/<day>/<time>/<id>",
        "view",
        viewNote,
        methods=["GET", "POST"],
    )
...
if __name__ == "__main__":
    ...
    app.run(
        debug=app.config["DEBUG"], host=app.config["HOST_IP"], port=app.config["PORT"]
    )
